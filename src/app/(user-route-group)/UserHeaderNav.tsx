...
import { useFirm, useUser } from '@/lib/queries';
...
const UserHeaderNav = () => {
  ...
  const { data: user, error: firmErr } = useUser(jwt?.uuid); //current user from DB
  const { data: firm, error: userErr } = useFirm(currentFirm?.id); //current firm from

  useEffect(() => {
    console.log(jwt?.workpapers.firms.length);
    console.log(user, firm);
  }, [currentFirm, user, firm, jwt]);
    ...
    <div className='flex items-center'>
      ...
      <div className='dropdown dropdown-hover dropdown-center'>
        <div
          tabIndex={0}
          role='button'
          className='btn m-0 border-blk bg-white border-0 rounded-none  hover:shadow-sm hover:bg-gray-100'
        >
          <svg
            xmlns='http://www.w3.org/2000/svg'
            fill='none'
